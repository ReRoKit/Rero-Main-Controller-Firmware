...
{
    SPI1ABUF = dat;
    xSystemState.bSdBusy = 1;
    while (!SPI1ASTATbits.SPIRBF);
    return (BYTE)SPI1ABUF;
}

void OpenSPI(unsigned int config1, unsigned int config2)
{
	SPI1ABRG=SpiBrgConvTblA[config1&0x1f];	// keep the prescaler 5 bits

	if( SPI1ASTATbits.SPIROV )
	{
		SPI1ASTATCLR=0x00000040;		// clear overrun bit 6
	}
...
BYTE MDD_SDSPI_ReadMedia(void)
{
    ...
    return (BYTE)xchg_spi((BYTE)0xFF);
