    ...
    switch(state)
    {
        ...
        case RNDBUTTON_DRAW:
			if (GetState(pB, BTN_NOPANEL))
            {
	        	...
	        	if (pB->pBitmap != NULL)
	        	{
                    ...
                    if (*(unsigned char*)(pB->pBitmap) != 0) {
                        xSemaphoreTake(xSdCardMutex,portMAX_DELAY);

                        // Show the image if the file can be opened.
                        FSFILE *pImageFile = FSfopen(pB->pBitmap, "r");
                        if (pImageFile != NULL) {
                            WORD wWidth = pB->hdr.right - pB->hdr.left;
                            WORD wHeight = pB->hdr.bottom - pB->hdr.top;
                            ImageDecode(pImageFile, IMG_BMP, pB->hdr.left, pB->hdr.top, wWidth, wHeight, IMG_ALIGN_CENTER, NULL, NULL);
                            FSfclose(pImageFile);
                        }
        ...
        case TEXT_DRAW:
            if(pB->pText != NULL)
            {
                ...
                {
                    yText = pB->hdr.bottom - pB->textHeight - 1;
                }
                else
                {
                    // centered	text in y direction
