...
{
    unsigned char i;
    static GOB_ID aeGidList[MAX_GOB];
    OBJ_HEADER  *pCurrentObj;
//    OBJ_HEADER  *pNextObj;
    WORD        translatedMsg;
    ...
    // Copy all the graphic objects ID into the list.
    // We do not want to process the newly created objects.
    i = 0;
    pCurrentObj = _pGolObjects;
    while(pCurrentObj != NULL) {
        aeGidList[i++] = pCurrentObj->ID;
        
        // Make sure the number of objects in one page is less than the maximum.
        if (i == MAX_GOB) {
            xSystemError.bObjectsOverflowError = 1;
            break;
        }
        else {
            // Set the next ID as invalid.
            aeGidList[i] = GID_INVALID;
        }
    ...
    // Process the message for all the objects in the list.
    for (i = 0; i < MAX_GOB; i++) {
        // Break if we have reached the end of the list.
        if (aeGidList[i] == GID_INVALID) {
            break;
        }
        
        pCurrentObj = GOLFindObject(aeGidList[i]);
        if (pCurrentObj != NULL) {
            if (pCurrentObj->MsgObj) {
                ...
                if (translatedMsg != OBJ_MSG_INVALID) {
                    if (GOLMsgCallback(translatedMsg, pCurrentObj, pMsg)) {
                        // Reload the current object. It might be deleted.
                        pCurrentObj = GOLFindObject(aeGidList[i]);
                        if (pCurrentObj != NULL) {
