...
BYTE MSDTasks(void)
{
    ...
    switch(MSD_State)
    {
        ...
        {
            // Detach the MSD if mass storage is not enabled.
            if (xSystemState.bUsbMassStorage == 0) {
                LUNSoftDetach(0);
            }
            else {
                LUNSoftAttach(0);
...
void MSDProcessCommandMediaPresent(void)
{
    ...
    switch(MSDCommandState)
    {
	    ...
	    case MSD_MODE_SENSE:
    	    ...
    	    break;
            case MSD_PREVENT_ALLOW_MEDIUM_REMOVAL:
                if ( (gblCBW.CBWCB[4] & 0x03) == 0x00 )   // check PREVENT field
                {                                         // success
                    ResetSenseData();
                } else {                                  // fail
            ...
            case MSD_VERIFY:
                ...
                break;
            case MSD_STOP_START:
                if ( (gblCBW.CBWCB[4] & 0x01) == 0x00 )   // check START field
                {                                         // STOP command
                    xSystemState.bUsbMassStorage = 0;
                }
                ResetSenseData();
                msd_csw.dCSWDataResidue=0x00;
                MSDCommandState = MSD_COMMAND_WAIT;
